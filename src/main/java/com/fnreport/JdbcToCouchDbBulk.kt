...
object JdbcToCouchDbBulk {
    ...
    fun go(args: Array<out String>) {
        ...
        val (connection, dbMeta) = connectToJdbcUrl(args)
        ...
        listOf(catalogg.value, schemaa.value, tablenamePattern.value).let { (cname, sname, tpat) ->
            ...
            rows.forEach {
                ...
                if (statement.execute("select count(*) from $tname")) {
                    var rowCount = statement.resultSet?.takeIf(ResultSet::next)?.getLong(1) ?: 0;
